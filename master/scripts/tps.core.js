...
var TopSource = 
{
	...
	_msg: {},
	_set: {
		loginUrl: "mod=public&fname=login",
		loginTitle: "用户登录",
		debug: false
	},
	...
	{	
		...
		if(json === null)
		{
			...
			setTimeout(function ()
			{
				location.href = TopSource.linkUrl.PANEL;
			}, 2000);
		}
		else
		{
			json = eval("(" + json + ")");
			$("#adminx").html(json.username);
			$("#totalx").html(json.logintotal);
			$("#timex").html(json.udatetiimes);
			$("#rolex").html(json.role);
			$("#ipx").html(json.ipaddress);
			$("#uid").html(json.username);
		}
	},
	debug: function (msg)
	{
	...
	loadLogin: function ()
	{
			if ($.pdialog && TopSource._set.loginTitle)
			{
				$.pdialog.open(TopSource._set.loginUrl, "login", TopSource._set.loginTitle, { mask: true, width: 520, height: 260 });
			} else
			{
				window.location = TopSource._set.loginUrl;
			}
	},
	obj2str: function (o)
	{
		...
		if (typeof o == "object")
		{
			...
			{
				for (var i in o)
						r.push(i + ":" + TopSource.obj2str(o[i]));
	...
	{
		if(json.callbackType != "void(0)")
		{
			if (json.statusCode == TopSource.statusCode.error)
			{
				if (json.message && alertMsg) 
					alertMsg.error(json.message);
			} 
			else if (json.statusCode == TopSource.statusCode.timeout)
			{
				if (alertMsg) 
					alertMsg.error(json.message || TopSource.msg("sessionTimout"), { okCall: TopSource.loadLogin });
				else 
					TopSource.loadLogin();
			} else
			{
	...
	init: function (pageFrag, options)
	{
		var op = $.extend(
		{
			loginUrl: "", loginTitle: null, callback: null, debug: false,
			statusCode: {}
		}, options);
		this._set.loginUrl = op.loginUrl;
		this._set.loginTitle = op.loginTitle;
		this._set.debug = op.debug;
		$.extend(TopSource.statusCode, op.statusCode);
		$.extend(TopSource.pageInfo, op.pageInfo);
		jQuery.ajax(
		{
			type: 'GET',
			url: pageFrag,
			...
			error: function (xhr)
			{
				alert('配置文件加载失败! 错误状态: ' + xhr.status + " 配置文件不存在");
			},
			success: function (xml)
			{
				$(xml).find("_PAGE_").each(function ()
				{
					...
					if (pageId) TopSource.frag[pageId] = $(this).text();
				});
				$(xml).find("_MSG_").each(function ()
				{
					var id = $(this).attr("id");
					if (id) TopSource._msg[id] = $(this).text();
				});
				if (jQuery.isFunction(op.callback)) op.callback();
			}
		});
		var _doc = $(document);
		if (!_doc.isBind(TopSource.eventType.pageClear))
		{
			_doc.bind(TopSource.eventType.pageClear, function (event)
			{
...
(function ($)
{
	...
	$.fn.extend(
	{
		...
		{
			var $this = $(this);
			$this.trigger(TopSource.eventType.pageClear);
			$.ajax(
			{
				type: op.type || 'GET',
				url: op.url,
				...
				{
					var json = TopSource.jsonEval(response);
					if (json.statusCode == TopSource.statusCode.error)
					{
					...
					if (json.statusCode == TopSource.statusCode.timeout)
					{
						if ($.pdialog) $.pdialog.checkTimeout();
						if (navTab) navTab.checkTimeout();
						alertMsg.error(json.message || TopSource.msg("sessionTimout"), { okCall: function ()
						{
							TopSource.loadLogin();
						} 
						});
				...
				error: TopSource.ajaxError,
				statusCode: 
				{
					503: function (xhr, ajaxOptions, thrownError)
					{
						alert(TopSource.msg("statusCode_503") || thrownError);
					} 
				}
			});
		},
			...
			$(op.toggleBut, bar).click(function ()
			{
				...
				$(op.container).animate({ left: cleft, width: cwidth }, 50, function ()
				{
					bar.animate({ left: barleft }, 500, function ()
					{
						...
						$(window).trigger(TopSource.eventType.resizeGrid);
					});
				});
...
function initEnv()
{
	$("body").append(TopSource.frag["dwzFrag"]);
	...
	{
		initLayout();
		$(this).trigger(TopSource.eventType.resizeGrid);
	});
...
function initLayout()
{
	var iContentW = $(window).width() - (TopSource.ui.sbar ? $("#sidebar").width() + 10 : 34) ;
	var iContentH = $(window).height() - 134;
	...
	$("#taskbar").css({ top: iContentH + 105, width: $(window).width() });
}
function initUI(_box)
{
	...
	if ($.fn.xheditor)
	{
		...
		{
			var $this = $(this);
			var op = { html5Upload: true, skin: 'vista', tools: $this.attr("tools") || 'full', upImgUrl: TopSource.linkUrl.HANDLE +"?m=upload&c=editor", upImgExt: "jpg,jpeg,gif,png,bmp"};
			var upAttrs = [];
			$(upAttrs).each(function (i)
			{
				var urlAttr = upAttrs[i][0];
				var extAttr = upAttrs[i][2];
	...
	$("form.required-validate", $p).each(function ()
	{
		...
		$form.validate(
		{
			...
			invalidHandler: function (form, validator)
			{
				...
				if (errors)
				{
					var message = TopSource.msg("validateFormError", [errors]);
	...
	$("*[target=navTab]", $p).each(function ()
	{
		$(this).click(function (event)
		{
			...
			var tabid = rel = $this.attr("rel") || "_blank";
			if (tabid == "desktop")
				tabid = "main";
			...
			var urls = TopSource.linkUrl.PANEL + "?" + lang;
			if (!urls.isFinishedTm())
			{
				alertMsg.error($this.attr("warn") || TopSource.msg("alertSelectMsg"));
				return false;
			}
			navTab.openTab(tabid, urls, { title: title, fresh: fresh, external: external });
			event.preventDefault();
		});
	});
	$("*[target=dialog]", $p).each(function ()
	{
		$(this).click(function (event)
		{
			TopSource.chkLogin();
			...
			if(!lang)
				return false;
			lang = lang.replaceTmById($(event.target).parents(".unitBox:first"));
			var urls = TopSource.linkUrl.PANEL + "?" + lang;
			if (!urls.isFinishedTm())
			{
				alertMsg.info($this.attr("warn") || TopSource.msg("alertSelectMsg"));
				return false;
			}
			$.pdialog.open(urls, rel, title, options);
			return false;
		});
	});
	$("*[target=adjust]", $p).each(function ()
	{
		$(this).click(function (event)
		{
			TopSource.chkLogin();
	...
	$("*[target=ajax]", $p).each(function ()
	{
		$(this).click(function (event)
		{
			...
			if(!lang)
				return false;
			lang = lang.replaceTmById($(event.target).parents(".unitBox:first"));
			var $callback = ajaxChkDone;                                               
			var url = TopSource.linkUrl.HANDLE + "?" + lang 
			$.ajax(
			{
				type: 'GET',
				url: url,
	...
	if ($.fn.ajaxTodo) $("*[target='ajaxTodo']", $p).ajaxTodo();
	if ($.fn.dwzExport) $("*[target='dwzExport']", $p).dwzExport();
	if ($.fn.lookup) $("*[lookupGroup]", $p).lookup();
	...
	if ($.fn.selectedTodo) $("*[target=selectedTodo]", $p).selectedTodo();
	if ($.fn.mulitTodo) $("*[target=mulitTodo]", $p).mulitTodo();
...
var alertMsg = 
{
	...
	{
		if (event.keyCode == TopSource.keyCode.ENTER) 
			event.data.target.trigger("click");
		return false;
	},
	_keydownEsc: function (event)
	{
		if (event.keyCode == TopSource.keyCode.ESC) 
			event.data.target.trigger("click");
	},
	_open: function (type, msg, buttons)
	{
		...
		if (buttons)
		{
			for (var i = 0; i < buttons.length; i++)
			{
				...
				butsHtml += TopSource.frag["alertButFrag"].replace("#butMsg#", buttons[i].name).replace("#callback#", sRel);
			} 
		}
		var boxHtml = TopSource.frag["alertBoxFrag"].replace("#type#", type).replace("#title#", this._getTitle(type)).replace("#message#", msg).replace("#butFragment#", butsHtml);
		...
		for (var i = 0; i < buttons.length; i++)
		{
			...
			if (buttons[i].keyCode == TopSource.keyCode.ENTER)
			{
				jDoc.bind("keydown", { target: jButs.eq(i) }, this._keydownOk);
			}
			if (buttons[i].keyCode == TopSource.keyCode.ESC)
			{
	...
	_alert: function (type, msg, options)
	{
		...
		$.extend(op, options);
		var buttons = [{ name: op.okName, call: op.okCall, keyCode: TopSource.keyCode.ENTER}];
	...
	confirm: function (msg, options)
	{
		...
		$.extend(op, options);
		var buttons = [{ name: op.okName, call: op.okCall, keyCode: TopSource.keyCode.ENTER }, { name: op.cancelName, call: op.cancelCall, keyCode: TopSource.keyCode.ESC}];
...
(function ($)
{
	...
	{
		var cur = hash[index];
		var content = $(TopSource.frag[cur.id]);
...
var navTab = 
{
	...
	_closeTab: function (index, openTabid)
	{
		...
		this._getPanels().eq(index).trigger(TopSource.eventType.pageClear).remove();
	...
	closeAllTab: function ()
	{
		...
		this._getPanels().filter(":gt(0)").trigger(TopSource.eventType.pageClear).remove();
	...
	_closeOtherTab: function (index)
	{
		...
		if (index > 0)
		{
			...
			this._getPanels().not(str$).filter(":gt(0)").trigger(TopSource.eventType.pageClear).remove();
	...
	{
		var json = TopSource.jsonEval(this.getCurrentPanel().html());
		if (json && json.statusCode == TopSource.statusCode.timeout) this.closeCurrentTab();
	},
	openExternal: function (url, $panel)
	{
		var ih = navTab._panelBox.height();
		$panel.html(TopSource.frag["externalFrag"].replaceAll("{url}", url).replaceAll("{height}", ih + "px"));
	},
	openTab: function (tabid, url, options)
	{
		...
		{
			var tabFrag = '<li tabid="#tabid#"><a href="javascript:" title="#title#" class="#tabid#"><span><span class="tabico">#title#</span></span></a><a href="javascript:;" class="close">close</a></li>';
			...
			this._moreBox.append('<li><a href="javascript:" title="#title#"><span>#title#</span></a></li>'.replaceAll("#title#", op.title));
			var $tabs = this._getTabs();
...
(function ($)
{
        $.pdialog = {
                ...
                open: function (url, dlgid, title, options)
                {
                        ...
                        {
                                $("body").append(TopSource.frag["dialogFrag"]);
                ...
                _init: function (dialog, options)
                {
                        ...
                        var iTop = ($(window).height() - dialog.height()) / 3;
                        dialog.css({
                ...
                initResize: function (resizable, dialog, target)
                {
                        ...
                        var op = $.extend({}, this._op, options);
                        $("body").css("cursor", op.resizable?target + "-resize":"default");
                        resizable.css({
                                top: $(dialog).css("top"),
                ...
                close: function (dialog)
                {
                        ...
                        if (close && $.isFunction(close))
                        {
                                ...
                                if (param && param != "")
                                {
                                        param = TopSource.jsonEval(param);
                                        go = close(param);
                                } else
                                {
                                        go = close();
                        ...
                        $(dialog).trigger(TopSource.eventType.pageClear).remove();
                },
				shut: function (dialog)
                {
                        if (typeof dialog == 'string') dialog = $("body").data(dialog);
                        var close = dialog.data("close");
                        var go = true;
                ...
                {
                        this.shut($.pdialog._current);
                },
                checkTimeout: function ()
                {
                        var $conetnt = $(".dialogContent", $.pdialog._current);
                        var json = TopSource.jsonEval($conetnt.html());
                        if (json && json.statusCode == TopSource.statusCode.timeout) this.closeCurrent();
                },
...
(function ($)
{
        ...
        $.dialogDrag = {
                ...
                _init: function (dialog)
                {
                        ...
                        if (!shadow.size())
                        {
                                shadow = $(TopSource.frag["dialogProxy"]);
                                $("body").append(shadow);
                        }
        ...
        var sortDrag = {
                ...
                _getOverSortBox: function ($item)
                {
                        ...
                        return $(_op.sortBoxs).filter(':visible').filter(function ()
                        {
                                ...
                                return TopSource.isOver(y, x, sortBoxPos.top, sortBoxPos.left, $sortBox.height(), $sortBox.width());
                        });
                } 
        };
...
(function ($)
{
        $.fn.jTable = function (options)
        {
                return this.each(function ()
                {
                        ...
                        $(window).unbind(TopSource.eventType.resizeGrid).bind("resizeGrid", _resizeGrid);
                });
        };
        $.jTableTool = {
...
(function ($)
{
        ...
        $.taskBar = {
                ...
                init: function (options)
                {
                        ...
                        {
                                this._taskBar = $(TopSource.frag["taskbar"]).appendTo($("#layout"));
...
function validateCallback(form, callback, confirmMsg)
{
	...
	var _submitFn = function ()
	{
		$.ajax(
		{
			...
			dataType: "json",
			cache: false,
			success: callback || TopSource.ajaxDone,
			error: TopSource.ajaxError
		});
	}
	if (confirmMsg)
...
function updateCallback(form, callback, confirmMsg)
{
	var $form = $(form);
	var _submitFn = function ()
	{
...
function iframeCallback(form, callback)
{
        ...
        _iframeResponse($iframe[0], callback || TopSource.ajaxDone);
}
...
{
	TopSource.ajaxDone(json);
	if (json.statusCode == TopSource.statusCode.ok)
	{
		if (json.navTabId)
		{
		...
		{
			alertMsg.confirm(json.confirmMsg || TopSource.msg("forwardConfirmMsg"), 
			{
				okCall: function ()
				{
					navTab.reload(json.forwardUrl);
		...
		else if ("void(0)" == json.callbackType)
		{
			void(0);
		}
		else
...
function dialogAjaxDoneFather(json)
{
	...
	if (json.statusCode == TopSource.statusCode.ok)
	{
		...
		{
			navTab.reload(json.forwardUrl, { navTabId: json.navTabId });			
			var dialog = $("body").data('construct');
			$.pdialog.reload(dialog.data("url"), null)
		} 
		
	}
}
function ajaxSltDone(json)
{
	TopSource.ajaxDone(json);
	if (json.statusCode == TopSource.statusCode.ok)
	{
...
function uploadAjaxDone(json)
{
	...
	if(json.statusCode==TopSource.statusCode.ok)
	{
		if(json.navTabId)
		{
			$("#"+json.navTabId).val(json.url);
		}
        if(json.rel)
        {
            $("#" + json.rel).attr("src",json.url);        
        }
	}
}
...
{
	var $form = $(form);
	if (form[TopSource.pageInfo.pageNum]) form[TopSource.pageInfo.pageNum].value = 1;
...
function _getPagerForm($parent, args)
{
	...
	if (form)
	{
		if (args["pageNum"]) form[TopSource.pageInfo.pageNum].value = args["pageNum"];
		if (args["numPerPage"]) form[TopSource.pageInfo.numPerPage].value = args["numPerPage"];
		if (args["orderField"]) form[TopSource.pageInfo.orderField].value = args["orderField"];
		if (args["orderDirection"] && form[TopSource.pageInfo.orderDirection]) form[TopSource.pageInfo.orderDirection].value = args["orderDirection"];
	}
	return form;
}
...
$.fn.extend(
{
	ajaxTodo: function ()
	{
		return this.each(function ()
		{
			...
			$this.click(function (event)
			{
				var urls = TopSource.linkUrl.HANDLE + "?" + lang;
				if (!urls.isFinishedTm())
				{
					alertMsg.info(TopSource.msg("alertSelectMsg"));
					return false;
				}
				...
				if (title)
				{
					alertMsg.confirm(title, 
					{
						okCall: function ()
						{							
							ajaxTodo(urls, $this.attr("callback"));
						}
					});
				} 
				else
	...
	$.fn.extend(
	{
		pagination: function (opts)
		{
			...
			return this.each(function ()
			{
				...
				$this.html(TopSource.frag["pagination"].replaceAll("#pageNumFrag#", pageNumFrag).replaceAll("#currentPage#", pc.getCurrentPage())).find("li").hoverClass();
				...
				$this.find(setting.jumpto$).each(function ()
				{
					...
					{
						if (event.keyCode == TopSource.keyCode.ENTER) $button.click();
					});
				});
			});
	...
	$.fn.extend(
	{
		lookup: function ()
		{
			return this.each(function ()
			{
				...
				$this.click(function (event)
				{
					...
					if(!lang)
						return false;
					var url = TopSource.linkUrl.PANEL + "?" + lang;
					url = url.replaceTmById($(event.target).parents(".unitBox:first"));
					if (!url.isFinishedTm())
					{
						alertMsg.error($this.attr("warn") || TopSource.msg("alertSelectMsg"));
						return false;
					}
					$.pdialog.open(url, "lookup", $this.attr("title") || $this.text(), options);
					return false;
				});
			});
		},
		multLookup: function ()
		{
			return this.each(function ()
			{
				...
				$this.click(function (event)
				{
					...
					{
						var _args = TopSource.jsonEval($(this).val());
		...
		suggest: function ()
		{
			...
			return this.each(function ()
			{
				...
				{
					if (event.keyCode == TopSource.keyCode.ENTER && $(op.suggest$).is(':visible')) 
						return false;
				});
				...
				function _show(event)
				{
					...
					if (!url.isFinishedTm())
					{
						alertMsg.error($input.attr("warn") || TopSource.msg("alertSelectMsg"));
						return false;
					}
					var postData = {};
					...
					$.ajax(
					{
						...
						success: function (response)
						{
							...
							if ($lis.size() == 1 && event.keyCode != TopSource.keyCode.BACKSPACE)
							{
								_select($lis.eq(0));
							} 
							else if ($lis.size() == 0)
							{
								...
								$.bringBackSuggest(TopSource.jsonEval(jsonStr));
							} 
						},
						error: function ()
						{
				...
				$input.focus(_show).click(false).keyup(function (event)
				{
					...
					{
						case TopSource.keyCode.ESC:
						case TopSource.keyCode.TAB:
						case TopSource.keyCode.SHIFT:
						case TopSource.keyCode.HOME:
						case TopSource.keyCode.END:
						case TopSource.keyCode.LEFT:
						case TopSource.keyCode.RIGHT:
								break;
						case TopSource.keyCode.ENTER:
								_close();
								break;
						case TopSource.keyCode.DOWN:
								if (selectedIndex >= $items.size() - 1) selectedIndex = -1;
								else selectedIndex++;
								break;
						case TopSource.keyCode.UP:
		...
		itemDetail: function ()
		{
			...
			function tdHtml(field)
			{
				...
				if (field.fieldAttrs)
				{
					var attrs = TopSource.jsonEval(field.fieldAttrs);
		...
		mulitTodo:function()
		{
			...
			return this.each(function ()
			{
				...
				if(!lang)
					return false;
				lang = lang.replaceTmById($(event.target).parents(".unitBox:first"));
				var selectedIds = $this.attr("idlist");
				var title = $this.attr("title") || $this.text();
				var options = {};
				var w = $this.attr("width");
				var h = $this.attr("height");
				...
				$this.click(function ()
				{
					...
					var idlist = _getIds(selectedIds, targetType);
					if (!idlist)
					{
						alertMsg.error($this.attr("warn") || TopSource.msg("alertSelectMsg"));
							return false;
					...
					var urls = TopSource.linkUrl.PANEL + "?m=" + lang + "&idlist="+ idlist;     
					if (title)
					{
						alertMsg.confirm(title, { okCall: function(){ $.pdialog.open(urls, rel, title, options)} });
					} else
					{
						 $.pdialog.open(urls, rel, title, options);
					}                       
		...
		selectedTodo: function ()
		{
			...
			return this.each(function ()
			{
				...
				$this.click(function ()
				{
					...
					var _callback = $this.attr("callback") || (targetType == "dialog" ? dialogAjaxDone : navTabAjaxDone);
					if (!$.isFunction(_callback)) _callback = eval('(' + _callback + ')');
					var urls = TopSource.linkUrl.HANDLE + "?" + lang + "&ids=" + ids;								
					var tps = ids.split(",");
					for(var i = 0;i < tps.length;i++)
					{
						var pid = $("#pid_" + tps[i]).val();
						sk += sk == "" ? pid:"," + pid;
					}							
					urls += "&pid=" + sk;
					function _doPost()
					{
						$.ajax(
						{
							type: 'POST', url: urls, dataType: 'json', cache: false,
							data: function ()
							{
								if (postType == 'map')
								{
...
(function ($)
{
        ...
        $.fn.datepicker = function (opts)
        {
                ...
                {
                        if (!((e.keyCode >= 48 && e.keyCode <= 57) || (e.keyCode == TopSource.keyCode.DELETE || e.keyCode == TopSource.keyCode.BACKSPACE))) { return false; } 
                }
                ...
                return this.each(function ()
                {
                        ...
                        $this.click(function (event)
                        {
                                ...
                                var iTop = offset.top + this.offsetHeight;
                                $(TopSource.frag['calendarFrag']).appendTo("body").css({
                                        left: offset.left + 'px',
                                        top: iTop + 'px'
	...
	$.extend($.fn, 
	{
			...
			return this.each(function ()
			{
				...
				$(op.selector, box).click(function ()
				{
					...
					if (options.is(":hidden"))
					{
						...
						if (top + options.height() > $(window).height() +1)
						{
		...
		combox: function ()
		{
			...
			return this.each(function (i)
			{
				...
				var value = $this.attr("curval")?$this.attr("curval"):$this.val();
				...
				if (ref && rel)
				{
					function _onchange(event)
					{
						...
						if(lang)
						{
							...
							tabid = $this.attr("rel");
							var urls = TopSource.linkUrl.PANEL + '?' + lang;
							urls = urls.replace("{value}", encodeURIComponent($this.attr("value")));
							navTab.openTab(tabid, urls, { title: title, fresh: true, external: false });
							event.preventDefault();
						}
						else
						{
							var $ref = $("#" + ref);
							$ref.removeAttr("curval");
							var urls = TopSource.linkUrl.PANEL + '?' + rel;
							urls = urls.replace("{value}", encodeURIComponent($this.attr("value")));
							
							if ($ref.size() == 0) 
								return false;
							$.ajax({
								type: 'POST', dataType: "json", url: urls, cache: false,
								data: {},
