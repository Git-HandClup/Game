...
<div class="pageContent">
	<form method="post" action="handle.php?m=attach&c=edit&a=adv&ids=<?php echo $ids?>&pos=0" class="pageForm required-validate" onsubmit="return validateCallback(this, dialogAjaxDone)" id="upform" enctype="multipart/form-data">
		<div class="pageFormContent" layoutH="51">
			<table class="task" width="100%">	
				<tr>					
					<td class="td1">广告类型：</td>
					<td class="td2">
						<input type="radio" class="cbx" name="fid" value="0"<?php echo $std->fid == 0 ?' checked="true"':''?>/>普通广告
						<input type="radio" class="cbx" name="fid" value="1"<?php echo $std->fid == 1 ?' checked="true"':''?>/>代码广告
				...
				<tr>
					...
					<td class="td2">
						<select name="cid" class="required digits combox" curval="<?php echo $std->cid?>">
						<?php
							$sql['length'] = 0;
							$sql['table'] = 'adzone';
							$sql['sort'] = 'pid asc';
				...
				<tr>
					...
					<td class="td2">
						<input type="text" name="sdatetimes" readonly="readonly" class="required short date" value="<?php echo date('Y-m-d',$std->sdatetimes)?>"/>
				...
				<tr>
					...
					<td class="td2">
						<input type="text" name="edatetimes" readonly="readonly" class="required short date" value="<?php echo date('Y-m-d',$std->edatetimes)?>"/>
				...
				<tr>
					...
					<td class="td2">
						<textarea style="height:60px" name="code"><?php echo $std->code?></textarea>
